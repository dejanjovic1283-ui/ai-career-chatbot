...
def retrieve_relevant_chunks(query: str, chunks, index, k: int = 3):
...
def ask_llm(context: str, question: str, chat_history: List[dict]) -> str:
    ...
    response = client.chat.completions.create(
        messages=cast(Iterable[ChatCompletionMessageParam], messages),  # Cast to expected type
        model="gpt-4o",
    )
    return response.choices[0].message.content or ""  # type: ignore  # Ensures str return
